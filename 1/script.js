...
function savePlatformSettings() {
    ...
    localStorage.setItem('90show_platform_settings', JSON.stringify(settings));
    applyPlatformSettings(settings);
    showToast('تم حفظ إعدادات المنصة بنجاح 💾');
}

function loadPlatformSettings() {
    const settings = JSON.parse(localStorage.getItem('90show_platform_settings'));
    if (settings) {
        const statusInput = document.getElementById('site-status-text');
        const titleInput = document.getElementById('site-title');

        if (statusInput) statusInput.value = settings.statusText;
        if (titleInput) titleInput.value = settings.siteTitle;

        applyPlatformSettings(settings); // Apply immediately on load
    }
}
...
function applyPlatformSettings(settings) {
    ...
    if (settings.statusText) {
        ...
        if (liveIndicator) {
            // Rebuild innerHTML to preserve icon and update text
        ...
        // Also update the header branding text
        const siteBranding = document.querySelector('.site-branding');
        if (siteBranding) {
            siteBranding.textContent = settings.statusText;
        }
    }
}
