...
function savePlatformSettings() {
    ...
    const settings = { statusText, siteTitle };
...
function toggleAdFormType() {
    ...
    if (type === 'slider' || type === 'dual') {
        ...
        urlLabel.textContent = type === 'video' ? "رابط اليوتيوب" : "رابط الصورة";
        linkWrapper.style.display = type === 'video' ? "none" : "block";
    }
}
...
async function saveAdCard() {
    ...
    if (type === 'slider' || type === 'dual') {
        ...
        const rows = document.querySelectorAll('#ad-slider-items-container .input-group');
        rows.forEach(row => {
...
function renderAd() {
    ...
    if (ad.type === 'slider') {
    ...
    } else if (ad.type === 'dual') {
        container.innerHTML = `
            <div class="ad-grid">
                ${ad.items.slice(0, 2).map((item, idx) => `
        ...
        const url = ad.items[0].url;
        const ytId = getYouTubeId(url);
        if (ytId) {
            container.innerHTML = `
                <div class="ad-box">
                    <iframe src="https://www.youtube.com/embed/${ytId}?autoplay=0&mute=1&loop=1&playlist=${ytId}" 
